#!/usr/bin/env python3
"""
FAZE Security API Integration for Splunk - Agentic Red Team
Fetches all vulnerabilities from FAZE Agentic Red Team platform
Supports deduplication by asset_name to avoid duplicate entries
"""
...
class FAZESecurityAddon:
    """FAZE Agentic Red Team vulnerability fetcher for Splunk"""

    def __init__(self, api_key: Optional[str] = None, api_url: str = "https://api.faze.security"):
    ...
    def run(self, art_id: int = 1000):
        ...
        if self.deduplicate:
            print("Deduplication enabled - showing only first occurrence per asset", file=sys.stderr)
        ...
        for vuln_group in vulns_grouped:
            ...
            for vuln_name, vuln_details in vuln_group.items():
                ...
                for item in items:
                    ...
                    # Deduplication: skip if we've already seen this vuln on this asset
        ...
        print(f"\nComplete! Fetched {self.vulns_count} vulnerabilities", file=sys.stderr)
        if self.deduplicate:
...
def main():
    ...
    try:
        addon = FAZESecurityAddon(api_key)
        addon.run(art_id)
    except Exception as e:
